...
from ..ebm_aligner import EBMAligner
...
class Res5ROIHeads(ROIHeads):
    ...
    def __init__(self, cfg, input_shape, feature_store=None):
        ...
        self.enable_ebm_aligner = cfg.EBM_ALIGNER.ENABLE
        self.ebm_feat_save_location = os.path.join(cfg.OUTPUT_DIR, cfg.EBM_ALIGNER.FEAT_SAVE_LOCATION)

        # EBM Parameters
        self.ebm_latent_dim = cfg.EBM_ALIGNER.EBM_LATENT_DIM
        self.ebm_n_layers = cfg.EBM_ALIGNER.EBM_N_LAYERS
        self.ebm_n_hidden_units = cfg.EBM_ALIGNER.EBM_N_HIDDEN_UNITS
        self.max_iter = cfg.EBM_ALIGNER.MAX_ITER
        self.ebm_lr = cfg.EBM_ALIGNER.EBM_LR
        self.n_langevin_steps = cfg.EBM_ALIGNER.N_LANGEVIN_STEPS
        self.langevin_lr = cfg.EBM_ALIGNER.LANGEVIN_LR
        self.ema_decay = cfg.EBM_ALIGNER.EMA_DECAY
        self.log_iter = cfg.EBM_ALIGNER.LOG_ITER
        self.batch_size = cfg.EBM_ALIGNER.BATCH_SIZE

        self.ebm_aligner = None
        if self.enable_ebm_aligner:
            if os.path.exists(self.ebm_feat_save_location) and len(os.listdir(self.ebm_feat_save_location)) > 0:
                print('Found {} features, ready to learn the EBM.'.format(len(os.listdir(self.ebm_feat_save_location))))
                self.ebm_aligner = EBMAligner(
                    ebm_latent_dim=self.ebm_latent_dim,
                    ebm_n_layers=self.ebm_n_layers,
                    ebm_n_hidden_units=self.ebm_n_hidden_units,
                    max_iter=self.max_iter,
                    ebm_lr=self.ebm_lr,
                    n_langevin_steps=self.n_langevin_steps,
                    langevin_lr=self.langevin_lr,
                    ema_decay=self.ema_decay,
                    log_iter=self.log_iter,
                    batch_size=self.batch_size)
                self.ebm_aligner.learn_ebm(self.ebm_feat_save_location)
            else:
                print('Feature files which are required to train the EBM is not found in {}'.
                      format(self.ebm_feat_save_location))
    ...
    def log_features(self, features):
        if not os.path.exists(self.ebm_feat_save_location):
            os.makedirs(self.ebm_feat_save_location)
        for feature in features:
            location = os.path.join(self.ebm_feat_save_location, shortuuid.uuid() + '.pkl')
            torch.save(feature, location)
    ...
    def forward(self, images, features, proposals, targets=None):
        ...
        # Store the features for training the EBM
        if self.training and self.enable_ebm_aligner:
            # Retrieve foreground proposals
            ...
            # Get the features
            prev_box_features = self.base_model.roi_heads.res5(boxes_fg)
            prev_feature_pooled = prev_box_features.mean(dim=[2, 3])
            prev_feature_pooled = prev_feature_pooled.detach().cpu().tolist()
            current_box_features = self.res5(boxes_fg)
            current_feature_pooled = current_box_features.mean(dim=[2, 3])
            current_feature_pooled = current_feature_pooled.detach().cpu().tolist()

            ebm_features = tuple(zip(current_feature_pooled, prev_feature_pooled))
            self.log_features(ebm_features)
        ...
        if not self.training and self.enable_ebm_aligner and self.ebm_aligner is not None:
            # Mask is True when its an old class instance
            mask = [p.gt_classes < self.num_base_class for p in proposals][0]
            # If its an old class feature, then align, else dont align
            feature_pooled_aligned = self.ebm_aligner.align_latents(feature_pooled)
            feature_pooled = torch.where(torch.ones_like(feature_pooled, dtype=bool).cuda() * mask[:, None],
                                         feature_pooled_aligned, feature_pooled)
