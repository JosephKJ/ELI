...
class GeneralizedRCNN(nn.Module):
    ...
    @torch.no_grad()
    def visualize_test_instance(self, batched_inputs, outputs):
        inputs = [x for x in batched_inputs]
        prop_boxes = [p for p in outputs]
        for input, prop in zip(inputs, prop_boxes):
            file_name = input['file_name'][-10:].split('.')[0]
            if file_name not in ['001138', '001392', '001478', '001551', '001652', '001696', '001824', '002074', '002121', '002227', '002503', '002582', '002650', '002792', '002925', '003590', '004219', '005754', '006083', '006771', '007567']:
                continue
            img = input["image"].detach().cpu().numpy()
            assert img.shape[0] == 3, "Images should have 3 channels."
            if self.input_format == "BGR":
                img = img[::-1, :, :]
            img = img.transpose(1, 2, 0)
            v = Visualizer(img, MetadataCatalog.get('voc_2007_trainval'), scale=1.2)
            v = v.draw_instance_predictions(prop['instances'].to('cpu'))
            img = v.get_image()[:, :, ::-1]
            cv2.imwrite('./images_selected/output_{}.jpg'.format(file_name), img)
            # exit(999)
    ...
    def inference(self, batched_inputs, detected_instances=None, do_postprocess=True):
        ...
        if do_postprocess:
            ...
            self.visualize_test_instance(batched_inputs, processed_results)
