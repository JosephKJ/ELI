...
def compute_accuracy(the_args, fusion_vars, b1_model, b2_model, tg_feature_model, class_means, \
    ...
    fast_fc=None, scale=None, print_info=False, device=None, cifar=True, imagenet=False, \
    valdir=None, aligner=None):
    if device is None:
    ...
    for batch_idx, (inputs, targets) in enumerate(evalloader):
        ...
        if aligner is not None:
            outputs, targets, outputs_feature = aligner.align_prev_classes(inputs, targets, b1_model,
                                                                           b2_model, fusion_vars, the_args,
                                                                           return_z=True)
        else:
...
def compute_task_wise_accuracy(the_args, fusion_vars, b1_model, b2_model, tg_feature_model, class_means, \
    X_protoset_cumuls, Y_protoset_cumuls, evalset, order_list, is_start_iteration=False, \
    fast_fc=None, scale=None, print_info=False, device=None, cifar=True, imagenet=False, \
    valdir=None, aligner=None, iteration=None, pin_memory=False):
    start_iter = int(the_args.nb_cl_fg/the_args.nb_cl)-1
    if iteration == start_iter:
        evalloader = torch.utils.data.DataLoader(evalset, batch_size=the_args.eval_batch_size,
                                                 shuffle=False, num_workers=the_args.num_workers,
                                                 pin_memory=pin_memory)
        cumul_acc, _ = compute_accuracy(the_args, fusion_vars, b1_model, b2_model, tg_feature_model, \
                                        class_means, X_protoset_cumuls, Y_protoset_cumuls, evalloader, order_list, \
                                        is_start_iteration=is_start_iteration, fast_fc=fast_fc, aligner=aligner)
        acc = cumul_acc[0]
        return [acc]
    else:
        number_of_tasks = iteration - start_iter
        acc = []
        for i in range(number_of_tasks+1):
            if i == 0:
                index = np.argwhere((np.array(evalset.targets) >= 0) & (np.array(evalset.targets) < the_args.nb_cl_fg)).squeeze()
            else:
                index = np.argwhere((np.array(evalset.targets) >= (the_args.nb_cl_fg + (i-1)*the_args.nb_cl)) &
                                    (np.array(evalset.targets) < (the_args.nb_cl_fg + i*the_args.nb_cl))).squeeze()

            # print("iteration: {}, number of tasks: {}, classes_in_current_task: {}".format(i, number_of_tasks,
            #                                                                                index))

            evalset_subset = torch.utils.data.Subset(evalset, index)
            evalloader = torch.utils.data.DataLoader(evalset_subset, batch_size=the_args.eval_batch_size,
                                                     shuffle=False, num_workers=the_args.num_workers,
                                                     pin_memory=pin_memory)
            cumul_acc, _ = compute_accuracy(the_args, fusion_vars, b1_model, b2_model, tg_feature_model,
                                            class_means, X_protoset_cumuls, Y_protoset_cumuls, evalloader, order_list,
                                            is_start_iteration=is_start_iteration, fast_fc=fast_fc, aligner=aligner)
            acc.append(cumul_acc[0])
        return acc
