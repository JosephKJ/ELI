...
class ResNet(nn.Module):
    ...
    def forward(self, x, return_z_also=False, return_gap_also=False):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        ...
        x = self.layer1(x)
        x = self.layer2(x)
        inner_x = self.layer3(x)

        x = self.avgpool(inner_x)
        z = x.view(x.size(0), -1)
        x = self.fc(z)

        if return_gap_also:
            # Returns Global Average Pool of the third conv layer, along with x and z.
            gap_features = inner_x.mean(dim=(2, 3))
            return x, z, gap_features
