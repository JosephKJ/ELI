""" Training code for Geodesic + LUCIR """
...
old_scores = []
new_scores = []
cur_features_geodesic = []
ref_features_geodesic = []
...
def get_ref_features_geodesic(self, inputs, outputs):
    global ref_features_geodesic
    ref_features_geodesic = inputs[0]
    ref_features_geodesic = F.adaptive_avg_pool2d(ref_features_geodesic, 2).view(ref_features_geodesic.size(0), -1)


def get_cur_features_geodesic(self, inputs, outputs):
    global cur_features_geodesic
    cur_features_geodesic = inputs[0]
    cur_features_geodesic = F.adaptive_avg_pool2d(cur_features_geodesic, 2).view(cur_features_geodesic.size(0), -1)
...
def incremental_train_and_eval(the_args, epochs, fusion_vars, ref_fusion_vars, b1_model, ref_model, b2_model,
                               ref_b2_model, \
                               tg_optimizer, tg_lr_scheduler, fusion_optimizer, fusion_lr_scheduler, trainloader,
                               testloader, iteration, \
                               start_iteration, X_protoset_cumuls, Y_protoset_cumuls, order_list, the_lambda, dist, \
                               K, lw_mr, balancedloader, fix_bn=False, weight_per_class=None, device=None, lamda=None):
    ...
    handle_ref_features_geodesic = ref_model.avgpool.register_forward_hook(get_ref_features_geodesic)
    handle_cur_features_geodesic = b1_model.avgpool.register_forward_hook(get_cur_features_geodesic)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            if iteration == start_iteration + 1:
                ...
                ref_outputs, ref_features_new = process_inputs_fp(the_args, ref_fusion_vars, ref_model, ref_b2_model,
                                                                  inputs)
                loss1 = nn.CosineEmbeddingLoss()(cur_features, ref_features_new.detach(),
                                                 torch.ones(inputs.shape[0]).to(device)) * the_lambda
            ...
            if hard_num > 0:
                ...
                loss3 = nn.MarginRankingLoss(margin=dist)(gt_scores.view(-1, 1), max_novel_scores.view(-1, 1),
                                                          torch.ones(hard_num * K).to(device)) * lw_mr
            else:
                loss3 = torch.zeros(1).to(device)

            # Loss 4: Geodesic loss
            loss4 = gfk.fit(ref_features_geodesic.detach(), cur_features_geodesic) * lamda  # Distillation loss
        ...
        print(
            'Train set: {}, train loss1: {:.4f}, train loss2: {:.4f}, train loss3: {:.4f}, train loss: {:.4f} accuracy: {:.4f}'.format(
                len(trainloader), train_loss1 / (batch_idx + 1), train_loss2 / (batch_idx + 1),
                train_loss3 / (batch_idx + 1), train_loss / (batch_idx + 1), 100. * correct / total))
    ...
    handle_ref_features_geodesic.remove()
    handle_cur_features_geodesic.remove()
