...
from utils.incremental.compute_accuracy import compute_task_wise_accuracy
...
class BaseTrainer(object):
    ...
    def set_dataset_variables(self):
        ...
        if self.args.dataset == 'cifar100':
            ...
            self.trainset = torchvision.datasets.CIFAR100(root=self.args.dataset_location, train=True, download=True, transform=self.transform_train)
            self.testset = torchvision.datasets.CIFAR100(root=self.args.dataset_location, train=False, download=True, transform=self.transform_test)
            self.evalset = torchvision.datasets.CIFAR100(root=self.args.dataset_location, train=False, download=False, transform=self.transform_test)
            self.balancedset = torchvision.datasets.CIFAR100(root=self.args.dataset_location, train=False, download=False, transform=self.transform_train)
            # Set the network architecture
            if self.args.enable_std_IL_setting:
    ...
    def init_class_order(self):
        ...
        if osp.exists(order_name) and not self.args.enable_std_IL_setting:
            ...
            if not self.args.enable_std_IL_setting:
    ...
    def create_train_loader(self, X_train, map_Y_train, drop_last=False):
        """The function to update the dataloaders
        Args:
          X_train: current-phase training samples, including new-class samples and old-class exemplars
          map_Y_train: mapped labels for X_train
        ...
        if self.args.dataset == 'cifar100':
            ...
            trainloader = torch.utils.data.DataLoader(train_set_temp, batch_size=self.args.train_batch_size,
                shuffle=True, num_workers=self.args.num_workers, drop_last=drop_last)
        elif self.args.dataset == 'imagenet_sub' or self.args.dataset == 'imagenet':
            # Set the training dataloader
            train_set_temp = copy.deepcopy(self.trainset)
            ...
            trainloader = torch.utils.data.DataLoader(train_set_temp, batch_size=self.args.train_batch_size,
                shuffle=True, num_workers=self.args.num_workers, pin_memory=True, drop_last=drop_last)
        else:
            raise ValueError('Please set correct dataset.')
        return trainloader
    ...
    def compute_acc(self, class_means, order, order_list, b1_model, b2_model, X_protoset_cumuls, Y_protoset_cumuls, \
        X_valid_ori, Y_valid_ori, X_valid_cumul, Y_valid_cumul, iteration, is_start_iteration, top1_acc_list_ori,
                    top1_acc_list_cumul, evaluate_with_ebm=False, aligner=None, top1_acc_with_alignment_list_cumul=None):
        ...
        task_wise_acc = compute_task_wise_accuracy(self.args, self.fusion_vars, b1_model, b2_model, tg_feature_model, \
            current_means, X_protoset_cumuls, Y_protoset_cumuls, self.evalset, order_list, \
            is_start_iteration=is_start_iteration, fast_fc=fast_fc, iteration=iteration, pin_memory=pin_memory)
        # print("task_wise_acc: {}".format(task_wise_acc))
        # Add the results to the array, which stores all previous results
        ...
        task_wise_acc_with_alignment = task_wise_acc
        if evaluate_with_ebm:
            if is_start_iteration:
                top1_acc_with_alignment_list_cumul[iteration, :, 0] = top1_acc_list_cumul[iteration, :, 0]
            else:
                # print('Computing cumulative accuracy with alignment...')
                evalloader = torch.utils.data.DataLoader(self.evalset, batch_size=self.args.eval_batch_size,
                                                         shuffle=False, num_workers=self.args.num_workers,
                                                         pin_memory=pin_memory)
                # Compute the accuracies for the current-phase data
                cumul_acc_with_alignment, _ = compute_accuracy(self.args, self.fusion_vars, b1_model, b2_model, tg_feature_model,
                                                current_means, X_protoset_cumuls, Y_protoset_cumuls, evalloader, order_list,
                                                is_start_iteration=is_start_iteration, fast_fc=fast_fc, aligner=aligner)
                task_wise_acc_with_alignment = compute_task_wise_accuracy(self.args, self.fusion_vars, b1_model, b2_model,
                                                           tg_feature_model,
                                                           current_means, X_protoset_cumuls, Y_protoset_cumuls,
                                                           self.evalset, order_list,
                                                           is_start_iteration=is_start_iteration, fast_fc=fast_fc, aligner=aligner,
                                                           iteration=iteration, pin_memory=pin_memory)
                # Add the results to the array, which stores all previous results
                top1_acc_with_alignment_list_cumul[iteration, :, 0] = np.array(cumul_acc_with_alignment).T
                # Write the results to tensorboard
                self.train_writer.add_scalar('cumul_acc_with_align/fc', float(cumul_acc_with_alignment[0]), iteration)
                self.train_writer.add_scalar('cumul_acc_with_align/proto', float(cumul_acc_with_alignment[1]), iteration)

        return top1_acc_list_ori, top1_acc_list_cumul, top1_acc_with_alignment_list_cumul, task_wise_acc, task_wise_acc_with_alignment

    def compute_fgt(self, data):
        """
        Given a TxT triangular data matrix, compute average forgetting at T-th task
        Adapted from: https://github.com/facebookresearch/agem/blob/main/utils/utils.py#L381
        """
        # Padding
        num_tasks = len(data)
        if num_tasks == 1:
        ...
        for index, acc in enumerate(data):
            acc_matrix.append(np.pad(acc, [(0, num_tasks - (index + 1))], mode='constant', constant_values=0))
